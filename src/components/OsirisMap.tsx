...
function OsirisMap({ data, activeLayers, onEntityClick, onMouseCoords, onRightClick, onViewStateChange, flyToLocation, highlight, projection = 'globe', mapStyle = 'dark', sweepData, scanTargets = [], demoMode = false, theme = 'core', initialCenter, initialZoom }: OsirisMapProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      // Alarm Vectors — unified wave-propagation flow lines (drone + missile).
      // A separate source lets downstream consumers query or style the combined
      // propagation data independently of the per-weapon-type route layers.
      map.addLayer({ id: 'alarm-vectors-flow', type: 'line', source: 'alarm-vectors',
        paint: { 'line-color': ['get', 'color'], 'line-width': 1.5, 'line-opacity': 0.5, 'line-dasharray': [2, 4] }});
      map.addLayer({ id: 'alarm-vectors-arrows', type: 'symbol', source: 'alarm-vectors',
        layout: { 'symbol-placement': 'line', 'text-field': '▶', 'text-size': 9, 'text-font': ['Open Sans Regular'], 'symbol-spacing': 100 },
