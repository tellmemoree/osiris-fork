...
const DISK_CACHE_TTL = 14400000; // 4 hours — TLE data is stale-tolerant (propagation degrades gracefully)

/** Save TLE data to disk so it survives server restarts */
function saveToDisk(sats: Tle[]) {
  try {
...
function loadFromDisk(): { sats: Tle[]; time: number } | null {
  try {
    if (!existsSync(CACHE_FILE)) return null;
    const data = JSON.parse(readFileSync(CACHE_FILE, 'utf-8'));
    // Accept disk cache up to 4 hours old
    if (data.sats?.length > 0 && Date.now() - data.time < DISK_CACHE_TTL) {
      return data;
    }
...
async function fetchCelesTrakGroup(url: string): Promise<Tle[]> {
  try {
